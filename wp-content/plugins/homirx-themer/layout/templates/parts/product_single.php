<?php 
   ...
   */
   ...
   $title = esc_html__('New Product Single Template', 'homirx-themer');
?>
<div class="gva-template-layout">
   <h3><?php echo esc_html__('Product Single Templates', 'homirx-themer') ?></h3>
   <div class="list-template-layout">
      ...
      <?php foreach ($this->get_templates($type) as $item) { ?>
         ...
         <div class="item" data-type="<?php echo esc_attr($type) ?>">
            <div class="state">
              <a class="btn-set-config-state <?php echo esc_attr($state) ?>" title="<?php echo esc_attr('Active Default', 'homirx-theme') ?>" href="#" data-id="<?php echo $item['id'] ?>" data-type="<?php echo esc_attr($type) ?>">
