<?php 
   ...
   */
   ...
   $title = esc_html__('New Product Archive Template', 'homirx-themer');
?>
<div class="gva-template-layout">
   <h3><?php echo esc_html__('Product Archive Templates', 'homirx-themer') ?></h3>
   <div class="list-template-layout">
      ...
      <?php foreach ($this->get_templates($type) as $item) { ?>
         ...
         <div class="item" data-type="<?php echo esc_attr($type) ?>">
            <div class="state">
              <a class="btn-set-config-state <?php echo esc_attr($state) ?>" href="#" title="<?php echo esc_attr('Active Default', 'homirx-theme') ?>" data-id="<?php echo $item['id'] ?>" data-type="<?php echo esc_attr($type) ?>">
               ...
               <?php if($languages && count($languages) > 1){ ?>
                  <?php 
                     foreach ($languages as $language){
                        if(!$language['active']){
                           echo '<a class="template-layout-dulipcate dulipcate-width-language" data-post_id="'.$item['id'].'" data-language="'.$language['code'].'" href="#" title="' . esc_attr('Duplicate to ', 'homirx-themer') . $language['code'] . '">';
