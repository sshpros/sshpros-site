...
class GVA_Layout_Action extends GVA_Layout_Model{
   ...
   public function ajax_auth_init(){ 
      add_action( 'wp_ajax_homirx_add_template_layout', array($this, 'add_template_layout') );
      add_action( 'wp_ajax_homirx_duplicate_template_layout', array($this, 'duplicate_template_layout') );
      add_action( 'wp_ajax_homirx_set_header_footer_layout', array($this, 'set_header_footer_layout') );
      add_action( 'wp_ajax_homirx_set_state_config', array($this, 'set_state_config') );
      add_action( 'wp_ajax_homirx_delete_template_layout', array($this, 'delete_template_layout') );
   }

   public function add_template_layout(){
      check_ajax_referer('homirx_ajax_security_nonce', 'security');
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( $name . " created", 'homirx-themer')
         )
      );
      die();
   }
   ...
   public function set_header_footer_layout(){
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Changes saved successfully", 'homirx-themer')
         )
      );
      die();
   }
   ...
   public function set_state_config(){
      ...
      if( empty($_POST['id']) || empty($_POST['type']) ){ 
         echo json_encode(
            array(
               'status'  => 'success',
               'message' => esc_html__( "Changes Unsuccess", 'homirx-themer')
            )
         );
         return;
      }
   ...
   public function duplicate_template_layout(){
      ...
      if (is_wp_error($new_post_id)) {
         echo json_encode(
            array(
               'status'  => 'success',
               'message' => esc_html__( "Can't Duplicate", 'homirx-themer')
            )
         );
         die();
      }
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Duplicate success", 'homirx-themer')
         )
      );
      die();
   ...
   function delete_template_layout(){
      ...
      if($template_type){
         ...
         if( count($get_templates) < 2){
            echo json_encode(
               array(
                  'status'  => 'unsuccess',
                  'message' => esc_html__( "Can't delete when have only one item", 'homirx-themer')
               )
            );
            die();
         }
      ...
      global $wpdb;
      $meta_key = 'homirx_template_layout';
      ...
      if( $count->THE_COUNT > 0 ){
         echo json_encode(
            array(
               'status'  => 'unsuccess',
               'message' => esc_html__( "Can't delete, some content use this template layout!", 'homirx-themer')
            )
         );
         die();
      }
      ...
      if (!$post) {
         echo json_encode(
            array(
               'status'  => 'unsuccess',
               'message' => esc_html__( "Can't Delete Template", 'homirx-themer')
            )
         );
         die();
      }
      ...
      echo json_encode(
         array(
            'status'  => 'success',
            'message' => esc_html__( "Template deleted", 'homirx-themer')
         )
      );
      die();
