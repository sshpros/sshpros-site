...
<div class="qodef-tab-wrapper <?php echo esc_attr( $class ); ?>">
	<ul class="qodef-tab-item-nav-wrapper">
		...
		foreach ( $this_object->get_children() as $child ) {
			...
			if ( ! empty( $dependency ) ) {
				$show     = array_key_exists( 'show', $dependency ) ? qode_essential_addons_framework_return_dependency_options_array( $child->get_scope(), $child->get_type(), $dependency['show'], true ) : array();
				$hide     = array_key_exists( 'hide', $dependency ) ? qode_essential_addons_framework_return_dependency_options_array( $child->get_scope(), $child->get_type(), $dependency['hide'] ) : array();
			...
			?>
			<li class="qodef-tab-item-nav-item <?php echo esc_attr( implode( ' ', $item_class ) ); ?>" <?php echo qode_essential_addons_framework_get_inline_attrs( $dependency_data, true ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
