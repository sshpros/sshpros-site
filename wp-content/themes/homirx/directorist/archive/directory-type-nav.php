...
<div class="directorist-type-nav">
	<ul class="directorist-type-nav__list">
		...
		<?php if ( ! empty( $all_types ) ) : ?>
			...
			<li class="list-inline-item <?php echo esc_attr(( $listings->current_listing_type === 'all' || 'all' === $current_directory_type ) ? 'current directorist-type-nav__list__current' : ''); ?>">
				<a class="directorist-type-nav__link" href="<?php echo esc_url( directorist_get_directory_type_nav_url( 'all' ) ); ?>">
				<?php directorist_icon( 'fa fa-grip-horizontal' ); ?> <?php esc_html_e( 'All', 'homirx' ); ?></a>
			</li>
		...
		<?php foreach ( $listings->listing_types as $id => $value ) : ?>
			...
			<li class="<?php echo ( ( $listings->current_listing_type === $value['term']->term_id && 'all' !== $current_directory_type ) ? 'current directorist-type-nav__list__current': '' ); ?>">
